...
SoL-Pi is developed and tested against `omp` 18.2.5, distributed as `@oh-my-pi/pi-coding-agent@18.2.5` and its sibling `@oh-my-pi/*` packages. Every mechanism is a standalone extension loaded through oh-my-pi's public extension APIs; upstream Pi is no longer a supported host. There is no dual-host mode, no compatibility shim for Pi, and no Pi specifier anywhere in `src/`.
...
- the `toolRenderers` registry from the `@oh-my-pi/pi-tui/tools` barrel, for the canonical `edit`/`write` transcript renderers
- the `Theme` type from the `@oh-my-pi/pi-tui/theme` subpath, for the renderer signatures and `themeOf()`
...
- **Tool renderers come from `@oh-my-pi/pi-tui/tools`.** omp 18.2.5 moved the terminal-UI modules — themes and tool renderers among them — out of `@oh-my-pi/pi-coding-agent`, and its root barrel does not re-export them. The `@oh-my-pi/pi-tui` root barrel re-exports the theme module, but not the renderers, which live on the `tools` barrel. Within `@oh-my-pi/pi-tui`, only the `tools` barrel is loadable from an extension at runtime: importing an individual `tools/<name>` module resolves under `bun test` but fails in a real `omp` run, because a compiled `omp` serves the host packages through bundled virtual modules whose graph reaches those modules by an unresolvable transitive edge. SoL-Pi therefore reads the canonical `edit`/`write` renderers from the `toolRenderers` registry on that barrel, which is the same map the transcript renders from.
- **`Theme` comes from the `@oh-my-pi/pi-tui/theme` subpath.** The coding-agent root reaches `Theme` only through `export * from "@oh-my-pi/pi-tui/theme"`, and the `@oh-my-pi/pi-tui` root barrel's own star-export of that class is not visible as an exported member under the resolutions that report `TS2305: Module '"@oh-my-pi/pi-tui"' has no exported member 'Theme'` — so `@oh-my-pi/pi-coding-agent` is the wrong specifier. The dedicated `@oh-my-pi/pi-tui/theme` subpath declares `export { Theme } from "./theme-class"` explicitly and is the specifier the host's own `.d.ts` files use. SoL-Pi therefore imports `Theme` from `@oh-my-pi/pi-tui/theme` throughout `src/` and `tests/`.
